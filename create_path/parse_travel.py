...
def main():
    # create triplet file
    # Get the arguments
    logger.info('..........Start parse_travel.py..................')
    args = docopt("""Parse the Travel questions and create a triplets file, each line is formatted as follows: X\t\Y\tpath

        Usage:
            parse_travel.py <travel_file> <vocabulary_file> <out_file>

            <travel_file> = the travel data file
    ...
    with codecs.open(vocabulary_file, 'r', 'utf-8') as f_in:
        ...
        logger.info("create vocabulary -->  return vocabulary words in a set")

    with codecs.open(travel_file, 'r', 'utf-8') as f_in:
        with codecs.open(out_file, 'w', 'utf-8') as f_out:
            ...
            for paragraph in f_in:
                ...
                # Skip empty lines
                # paragraph = get paragraphs in travel data
                paragraph = paragraph.strip()
                if len(paragraph) == 0:
                    continue
                parsed_par = nlp(unicode(paragraph))

                # parsed_par.sent --> sentence in a paragraph
                # Parse each sentence separately
                for sent in parsed_par.sents:
                    # (sentence, vocabulary file)
...
def parse_sentence(sent, vocabulary):
    """
        ...
        """
    logger.info('run parse_sentense to get all the dependency paths between nouns in the sentence')

    # Get all term indices
    # enumerate --> loop over something by getting indexes
    # (i, token, lemma, orth, pos) --> (7, apartments, u'apartment', u'apartments', u'NOUN')
    # index eg from indices:
    #   What are the best places around Cape Town for safari?
    #   [(u'be', are, 1, 1), (u'good', best, 3, 3), (u'place', places, 4, 4), (u'safari', safari, 9, 9)]
    ...
    # Iterate over the base noun phrases --> noun_chunks
    ...
    # tokens = (u'be', are, u'special', special)
